...
public class ExpressionBuilder {
    ...
    public Expression buildExpression(NonterminalParseTreeNode ptNode) {
        ...
        if (ptNode.getKind().toString().equals("Parser.TreeKind(full expression)")) {
            ptNode = (NonterminalParseTreeNode) ptNode.getChildren().get(0);
        }

        if (ptNode.getKind().toString().equals("Parser.TreeKind(delimited expression)")) {
        ...
        return switch (ptNode.getKind().toString()) {
            case "Parser.TreeKind(file)" -> buildFileExpression(ptNode);
            case "Parser.TreeKind(let)" -> buildLetExpression(ptNode);
            case "Parser.TreeKind(return)" -> buildReturnExpression(ptNode);
            case "Parser.TreeKind(lambda)" -> buildLambdaExpression(ptNode);
            default -> {
